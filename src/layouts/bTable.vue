    ...
    <table class="bMember_table table-striped">
        ...
        <tbody>
            <tr v-show="backNow=='聯絡表單'" v-for= "(data, key) in contact_data" :key="key">
                ...
                <td><button :class="{ 'red': data.STATUS === 0, 'green': data.STATUS === 1 }" @click="click_function(key,data.ID,'contact')">{{ data.STATUS === 1 ?  '未處理' : '已處理' }} </button></td>
            </tr>
            ...
            <tr v-show="backNow=='服務訂單'" v-for= "(data, key) in sOrder_data" :key="key">
                ...
                <td>{{ data.payment}}</td>
                <td><button :class="{ 'red': data.order_status === 0, 'green': data.order_status === 1 }" @click="click_function(key,data.ID,'pOrder')">{{ data.order_status === 1 ?  '未完成' : '已完成' }} </button></td>
            ...
            <tr v-show="backNow=='商品訂單'" v-for= "(data, key) in pOrder_data" :key="key">
                ...
                <td>{{ data.order_date }}</td>
                <td>{{ data.payment }}</td>
                <td><button :class="{ 'red': data.order_status === 0, 'green': data.order_status === 1 }" @click="click_function(key,data.ID,'pOrder')">{{ data.order_status === 1 ?  '配送中' : '已配送' }} </button></td>
                <td><button :class="{ 'red': data.rank_status === 0, 'green': data.rank_status === 1 }" @click="click_function(key,data.ID,'pOrder')">{{ data.rank_status === 1 ?  '退貨中' : '已退貨' }} </button></td>
